...
export interface User {
  ...
  bmi?: number;
  bmi_status?: string;
  healthy_bmi_low?: number;
  healthy_bmi_high?: number;
...
export class UserRepository {
  ...
  public upsertUser(user: Partial<User> & { telegram_id: number }): void {
  ...
  public clearProfile(telegram_id: number): void {
    const stmt = this.db.prepare(`
      UPDATE users
      SET
        display_name = NULL,
  ...
  public deleteUser(telegram_id: number): void {
    const stmt = this.db.prepare('DELETE FROM users WHERE telegram_id = ?');
    stmt.run(telegram_id);
  }

  public getUser(telegram_id: number): User | undefined {
    const stmt = this.db.prepare('SELECT * FROM users WHERE telegram_id = ?');
    return stmt.get(telegram_id) as User | undefined;
  }

  public getProfile(telegram_id: number): UserProfile | null {
  ...
  public updateTimezone(telegram_id: number, timezone: string): void {
  ...
  public updateDailyCalorieGoal(telegram_id: number, daily_calorie_goal: number): void {
    const stmt = this.db.prepare(
      'UPDATE users SET daily_calorie_goal = ?, updated_at = CURRENT_TIMESTAMP WHERE telegram_id = ?'
    );
